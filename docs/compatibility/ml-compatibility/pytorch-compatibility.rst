...
AMD provides pre-configured Docker images with PyTorch and the ROCm backend.
These images are published on `Docker Hub <https://hub.docker.com/r/rocm/pytorch>`__ and are the
recommended way to get started with deep learning on ROCm.

To find the right image tag for your environment, see the `PyTorch on ROCm installation documentation
<https://advanced-micro-devices-demo--593.com.readthedocs.build/projects/install-on-linux-internal/en/593/install/3rd-party/pytorch-install.html#docker-image-support>`__
for a list of available ``rocm/pytorch`` images.
